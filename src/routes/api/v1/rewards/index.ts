...
import { getPendingRewards, postRewardsAnalysis, deletePendingReward, getRewardsAnalysisStatus } from "./routes";
...
async function generateRewardsInBackground(job_id: string, community_id: string, platform_id: string, start_date: number, end_date: number) {
  try {
...
rewardsRoute.openapi(deletePendingReward, async (c) => {
  try {
    const { community_id } = c.req.query();
    ...
    const result = await db
      ...
      .where(and(
        ...
        eq(pendingRewards.community_id, community_id)
      ));
